...
**Files**: 10 files, ~900+ lines of code
...
- **Composite Tweens** - TweenSequence (TweenParallel removed)
...
### 📁 New File Structure (6 files, ~500 lines)
...
└── TweenPool.cs      # Object pooling (50 lines)
```
...
Tween.Parallel(
    ...
    other.TweenScale(targetScale, 1f)
);

// Loops
transform.TweenRotation(Quaternion.Euler(0, 360, 0), 2f)
        .Loops(3, LoopType.Yoyo);
```
...
- **TweenSequence class** - Simple chaining instead
...
- **Files**: 50% reduction (10 → 6 files) - TweenParallel removed
