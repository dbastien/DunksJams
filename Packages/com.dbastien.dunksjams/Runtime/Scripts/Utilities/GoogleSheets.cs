...
public static class GoogleSheets
{
    ...
    public static Dictionary<string, string> ParseCsv(string data, string langCode)
    {
        ...
        string[] headers = data[..lineEndIndex].Trim().Split(',');
    ...
    private static Dictionary<string, string> ParseData(string data, char delimiter, int column = 1)
    {
    ...
    public static async Task<Dictionary<string, string>> FetchGoogleSheetAsync(string sheetId, string range)
    {
        ...
        catch (HttpRequestException e) { DLog.LogE($"Network error while fetching Google Sheets data: {e.Message}"); }
        catch (Exception e) { DLog.LogE($"Failed to parse Google Sheets data: {e.Message}"); }
