...
public class StringFast
{
    ...
    int _pos;
    int _capacity;
    List<char> _replacement;

    object _valueControl;
    int _valueControlInt = int.MinValue;
    ...
    public void Set(string str) { Clear(); Append(str); _generated = str; _isGenerated = true; }
    public void Set(object str) => Set(str.ToString());
    public void Set<T1, T2>(T1 a, T2 b) { Clear(); Append(a); Append(b); }
    public void Set<T1, T2, T3>(T1 a, T2 b, T3 c) { Clear(); Append(a); Append(b); Append(c); }
    public void Set<T1, T2, T3, T4>(T1 a, T2 b, T3 c, T4 d) { Clear(); Append(a); Append(b); Append(c); Append(d); }

    public void Set(params object[] parts)
    {
        Clear();
        for (int i = 0; i < parts.Length; i++) Append(parts[i]);
    }
    ...
    public StringFast Clear() { _pos = 0; _isGenerated = false; return this; }
    ...
    {
        EnsureCapacity(16);
        if (value < 0) { value = -value; _buffer[_pos++] = '-'; }
    ...
    public StringFast Append(float valueF)
    {
        ...
        if (value == 0) { _buffer[_pos++] = '0'; return this; }
        if (value < 0) { value = -value; _buffer[_pos++] = '-'; }

        int nbDecimals = 0;
        while (value < 1000000) { value *= 10; nbDecimals++; }
    ...
    void EnsureCapacity(int charsToAdd)
    {
    ...
    static void AppendPaddedTwo(StringFast sf, int val, int max)
    {
        if (val > max) sf.Append("--");
        else if (val >= 10) sf.Append(val);
        else { sf.Append("0"); sf.Append(val); }
    }

    static void AppendPaddedThree(StringFast sf, int val)
    {
        if (val > 999) sf.Append("---");
        else if (val > 99) sf.Append(val);
        else if (val > 9) { sf.Append("0"); sf.Append(val); }
        else { sf.Append("00"); sf.Append(val); }
    }
}
