...
namespace Utilities
{
    public static class ReflectionUtils
    {
        ...
        static readonly HashSet<string> ExcludedPropertyNames =
        ...
        static Lazy<IEnumerable<Type>> _allTypes =
        ...
        {
            for (var openPos = name.IndexOf('['); openPos >= 0; openPos = name.IndexOf('[', openPos + 1))
            {
                var closePos = FindBracket(name.AsSpan(), openPos);
    ...
    #region attributes
        ...
        public static Dictionary<MemberInfo, List<Attribute>> GetMembersWithAttributes(this Type t,
            ...
            t.GetMembers(flags)
                .Select(member => new
                {
                    ...
                    attributes = attributeTypes
                        .SelectMany(attrType => member.GetCustomAttributes(attrType, true).Cast<Attribute>()).ToList()
                })
