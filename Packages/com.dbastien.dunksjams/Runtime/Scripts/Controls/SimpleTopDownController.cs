...
public class SimpleTopDownController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            Vector3 f = Vector3.ProjectOnPlane(cameraRoot.forward, Vector3.up).normalized;
            Vector3 r = Vector3.ProjectOnPlane(cameraRoot.right, Vector3.up).normalized;
            dir = (f * move.y + r * move.x).normalized;
        }
        else { dir = new Vector3(move.x, 0f, move.y).normalized; }
