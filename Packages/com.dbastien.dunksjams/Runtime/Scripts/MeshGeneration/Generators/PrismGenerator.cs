...
public static class PrismGenerator
{
    ...
    (
        int sides = 6, float radius = 0.5f, float height = 1f,
    ...
    )
    {
        ...
        for (var i = 0; i < sides; ++i)
        {
            ...
            float a1 = (i + 1) % sides * angleStep;
            float c0 = MathF.Cos(a0);
            float s0 = MathF.Sin(a0);
            float c1 = MathF.Cos(a1);
            float s1 = MathF.Sin(a1);

            Vector3 bl = new Vector3(c0 * radius, 0f, s0 * radius) + pivotOff;
            Vector3 br = new Vector3(c1 * radius, 0f, s1 * radius) + pivotOff;
            ...
            verts[vi] = bl;
            uvs[vi] = new Vector2(u0, 0f);
            vi++;
            verts[vi] = br;
            uvs[vi] = new Vector2(u1, 0f);
            vi++;
            verts[vi] = tl;
            uvs[vi] = new Vector2(u0, 1f);
            vi++;
            verts[vi] = tr;
            uvs[vi] = new Vector2(u1, 1f);
            vi++;
        ...
        for (var i = 0; i < sides; ++i)
        {
            ...
            int next = botCenter + 1 + (i + 1) % sides;
            tris[ti] = botCenter;
            tris[ti + 1] = vi;
            tris[ti + 2] = next;
            ti += 3;
        ...
        for (var i = 0; i < sides; ++i)
        {
            ...
            int next = topCenter + 1 + (i + 1) % sides;
            tris[ti] = topCenter;
            tris[ti + 1] = next;
            tris[ti + 2] = vi;
            ti += 3;
