...
public static class CapsuleGenerator
{
    ...
    )
    {
        ...
        float capsuleRadius = radius + cylinderHeight / 2f;
        ...
        for (var r = 0; r < midRing; ++r)
        {
            ...
            for (var s = 0; s < sectors; ++s)
            {
                ...
                Vector2 uv = MeshBuilder.SphericalUV(verts[vi] - pivotOff);
        ...
        if (cylinderHeight > 0f)
        {
            ...
            float heightStep = cylinderHeight / heightSegments;
            var bottom = new Vector3(0f, -cylinderHeight / 2f, 0f);
            float sinMid = MathF.Sin(MathF.PI * (midRing - 1) * R);
            ...
            for (var s = 0; s <= segments; ++s)
            {
                ...
                for (var j = 0; j <= heightSegments; ++j)
                {
                    ...
                    normals[cylVi] = dir;
                    Vector2 uv = MeshBuilder.SphericalUV(verts[cylVi] - pivotOff);
