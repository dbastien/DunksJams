...
public static class SuperEllipsoidGenerator
{
        ...
        PivotPosition pivot = PivotPosition.Center)
    {
        ...
        {
            var idx = j * (segments + 1) + i;
            var theta = i * 2f * MathF.PI / segments;
            var phi = -0.5f * MathF.PI + MathF.PI * j / (segments / 2f);
            ...
            uvs[idx].y = 0.5f + MathF.Atan2(verts[idx].y,
                MathF.Sqrt(verts[idx].x * verts[idx].x + verts[idx].z * verts[idx].z)) / MathF.PI;
        ...
        for (var i = 0; i < segments; ++i)
        {
            ...
            tris[ti] = i3; tris[ti + 1] = i2; tris[ti + 2] = i1;
            tris[ti + 3] = i4; tris[ti + 4] = i3; tris[ti + 5] = i1;
            ti += 6;
        }
    ...
    static float RPower(float v, float n) => v >= 0 ? MathF.Pow(v, n) : -MathF.Pow(-v, n);
}
