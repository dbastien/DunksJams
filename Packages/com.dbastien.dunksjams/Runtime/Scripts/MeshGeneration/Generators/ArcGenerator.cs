...
public static class ArcGenerator
{
    ...
    )
    {
        ...
        Vector3 cp = controlPoint ?? new Vector3(0f, height * 1.5f, 0f);
        ...
        for (var i = 0; i <= arcSegments; ++i)
        {
            ...
            verts[vi] = frontCurve[i];
            uvs[vi] = new Vector2(u, 0f);
            vi++;
            verts[vi] = backCurve[i];
        ...
        // Base quad
        Vector3 bl = new Vector3(-hw, 0, -hd) + pivotOff;
        Vector3 br = new Vector3(hw, 0, -hd) + pivotOff;
        Vector3 fr = new Vector3(hw, 0, hd) + pivotOff;
        Vector3 fl = new Vector3(-hw, 0, hd) + pivotOff;
        int bv = vi;
        verts[vi] = bl;
        ...
        uvs[vi] = new Vector2(1, 0);
        vi++;
        verts[vi] = frontCurve[0];
        uvs[vi] = new Vector2(0, 1);
        vi++;
        verts[vi] = backCurve[0];
        uvs[vi] = new Vector2(1, 1);
        vi++;
        tris[ti] = bv + 3;
        tris[ti + 1] = bv + 2;
        ...
        uvs[vi] = new Vector2(1, 0);
        vi++;
        verts[vi] = frontCurve[arcSegments];
        uvs[vi] = new Vector2(0, 1);
        vi++;
        verts[vi] = backCurve[arcSegments];
        uvs[vi] = new Vector2(1, 1);
        vi++;
        tris[ti] = bv;
        tris[ti + 1] = bv + 2;
        ...
        {
            float u = (float)i / arcSegments;
            Vector3 bottom = Vector3.Lerp(fl, fr, u);
            verts[vi] = bottom;
            uvs[vi] = new Vector2(u, 0f);
            vi++;
            verts[vi] = backCurve[i];
    ...
    private static Vector3 BezierQuadratic(Vector3 p0, Vector3 p1, Vector3 cp, float t)
    {
