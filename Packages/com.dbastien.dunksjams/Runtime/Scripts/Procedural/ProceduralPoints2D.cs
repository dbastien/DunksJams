...
public static class ProceduralPoints2D
{
    ...
    {
        var edge = Rand.Float() * (2f * width + 2f * height);
    ...
    {
        var angle = Rand.Rad();
        var r = MathF.Pow(Rand.Float(), 1f / n);
        return new Vector2(r * width * MathF.Cos(angle), r * height * MathF.Sin(angle));
    }
    ...
    {
        var angle = Rand.Rad();
        var r = MathF.Sqrt(Rand.Float() * (outerR * outerR - innerR * innerR) + innerR * innerR);
    ...
    {
        var angle = Rand.Rad();
        var r = Mathf.Lerp(innerR, outerR, Rand.Float());
        return new Vector2(r * MathF.Cos(angle), r * MathF.Sin(angle));
    }
    ...
    {
        var angle = Rand.Rad();
        var radius = MathF.Sqrt(Rand.Float() * (outerR * outerR - innerR * innerR) + innerR * innerR);
    ...
    {
        var theta = Mathf.Lerp(thetaMin, thetaMax, Rand.Float());
        var r = (a + b * theta) * Rand.Radial();
    ...
    {
        var angle = Rand.Rad();
        var sectorAngle = MathConsts.Tau / sides;
        var randomAngle = MathF.Floor(angle / sectorAngle) * sectorAngle;
    ...
    {
        var x = Mathf.Lerp(xMin, xMax, Rand.Float());
        var y = a * x * x + b * x;
    ...
    {
        var theta = Mathf.Lerp(thetaMin, thetaMax, Rand.Float());
        var r = a * MathF.Sin(theta) * MathF.Sin(theta) / MathF.Cos(theta);
        return new Vector2(r * MathF.Cos(theta), r * MathF.Sin(theta));
    }
    ...
    {
        var theta = Mathf.Lerp(thetaMin, thetaMax, Rand.Float());
        var r = 4f * a * MathF.Cos(theta) * MathF.Cos(theta) * MathF.Cos(theta);
    ...
    {
        var theta = Mathf.Lerp(thetaMin, thetaMax, Rand.Float());
        var r = amp * MathF.Cos(petalCount * theta);
        return new Vector2(r * MathF.Cos(theta), r * MathF.Sin(theta));
    }
