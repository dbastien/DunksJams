...
public class ProceduralPoints3D
{
    ...
    public static Vector3 InSphere(float radius)
    {
        var theta = Rand.Rad();
        var phi = Rand.Rad();
        var r = radius * MathF.Pow(Rand.Float(), 1f / 3f);
    ...
    {
        var edge = Rand.Float() * 3f;
        var offset = size * (Rand.Float() - 0.5f);
        return edge switch
        {
    ...
    {
        var edge = Rand.Float() * (2f * width + 2f * height + 2f * depth);
    ...
    {
        var angle = Rand.Rad();
        var cosSpread = MathF.Cos(spread * Rand.Float());
        var sinSpread = MathF.Sqrt(1f - cosSpread * cosSpread);
    ...
    {
        var angle = Rand.Rad();
        var cosSpread = MathF.Cos(spread);
        var sinSpread = MathF.Sqrt(1f - cosSpread * cosSpread);
        return new Vector3(sinSpread * MathF.Cos(angle), sinSpread * MathF.Sin(angle), cosSpread * rad);
    }
    ...
    public static Vector3 InCapsule(float radius, float height)
    {
        var angle = Rand.Rad();
        var r = radius * Rand.Radial();
        var z = Rand.Float() * height - height * 0.5f;
    ...
    {
        var theta = Rand.Rad();
        var r = tubeR * MathF.Sin(q * theta) + radius;
    ...
    {
        var theta = Rand.Rad() * revolutions;
        var r = Rand.Float() * radius * theta / MathConsts.Tau;
    ...
    {
        var theta = Rand.Rad() * revolutions;
        var r = radius * theta / MathConsts.Tau;
        return new Vector3(r * MathF.Cos(theta), r * MathF.Sin(theta), Rand.Float() * height - height * 0.5f);
    }
    ...
    {
        var phi = idx * MathConsts.GoldenRatio % MathConsts.Tau;
        var y = 1f - 2f * idx / (idx + 1).Fibonacci();
        var r = radius * MathF.Sqrt(1f - y * y) * MathF.Pow(Rand.Float(), 1f / 3f);
    ...
    public static Vector3 OnFibonacciSphere(float radius, int idx)
    {
        var phi = idx * MathConsts.GoldenRatio % MathConsts.Tau;
        var y = 1f - 2f * idx / (idx + 1).Fibonacci();
        var r = radius * MathF.Sqrt(1f - y * y);
        return new Vector3(r * MathF.Cos(phi), y * radius, r * MathF.Sin(phi));
    }

    public static Vector3 InFibonacciStructure3D(float radius, int index, FibonacciStructureType type)
    {
        var phi = index * MathConsts.GoldenRatio % MathConsts.Tau;
        var y = 1f - 2f * index / (index + 1).Fibonacci();
        var r = radius * MathF.Sqrt(1f - y * y);
