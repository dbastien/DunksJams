...
public class Eigen
{
    ...
    private float[] diagonal = new float[3];
    private float[,] matrix = new float[3, 3];
    private float[] subDiagonal = new float[3];
    public Vector3 Origin;

    private void MakeEnergyMatrix(bool isLinear, List<Vector3> points)
    {
        Origin = Vector3.zero;
        foreach (Vector3 v in points) Origin += v;
        Origin /= points.Count;
    ...
    private bool QlAlgorithm()
    {
        ...
        for (var i0 = 0; i0 < 3; ++i0)
        {
            ...
            for (i1 = 0; i1 < maxIter; ++i1)
            {
                int i2;
                for (i2 = i0; i2 <= 1; ++i2)
                    if (Mathf.Abs(subDiagonal[i2]) + Mathf.Abs(diagonal[i2]) + Mathf.Abs(diagonal[i2 + 1]) ==
                        Mathf.Abs(diagonal[i2]) + Mathf.Abs(diagonal[i2 + 1]))
                        break;
                ...
                for (int i3 = i2 - 1; i3 >= i0; --i3)
                {
                    ...
                    {
                        cos = g / f;
                        r = Mathf.Sqrt(cos * cos + 1f);
                        subDiagonal[i3 + 1] = f * r;
                        sin = 1f / r;
                        cos *= sin;
                    }
                    else
                    {
                        sin = f / g;
                        r = Mathf.Sqrt(sin * sin + 1f);
                        subDiagonal[i3 + 1] = g * r;
                        cos = 1f / r;
                        sin *= cos;
                    }
    ...
    public void SolvePlaneFor(List<Vector3> points)
    {
        MakeEnergyMatrix(false, points);
        Solve();
    }

    public void SolveLineFor(List<Vector3> points)
    {
        MakeEnergyMatrix(true, points);
        Solve();
    }
