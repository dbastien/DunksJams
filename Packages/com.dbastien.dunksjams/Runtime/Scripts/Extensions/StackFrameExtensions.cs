...
public static class StackFrameExtensions
{
    ...
    public static string FullInfo(this StackFrame sf) =>
        $"{sf.FileName()}:{sf.LineNumber()}:{sf.ColumnNumber()} ({sf.ClassName()}::{sf.MethodName()})";

    public static string ToString(this StackFrame sf) =>
        $"{sf.FileName()}:{sf.LineNumber()}:{sf.ColumnNumber()} ({sf.ClassName()}::{sf.MethodName()})";
}