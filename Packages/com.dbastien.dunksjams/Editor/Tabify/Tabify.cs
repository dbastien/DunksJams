...
public static class Tabify
{
        ...
        (Rect rect, string iconName, int iconSize, Color colorNormal, Color colorHovered, Color colorPressed)
    {
        ...
        SetGUIColor(c: color);
        float drawSize = Mathf.Min(iconSize, rect.width, rect.height);
        GUI.DrawTexture(rect.SetSizeFromMid(drawSize), icon.image);
        ResetGUIColor();
    ...
    private static void UpdateGUIs()
    {
        foreach (Object dockArea in AllDockAreas)
            if (!GuisByDockArea.ContainsKey(key: dockArea))
                GuisByDockArea[key: dockArea] = new TabifyGUI(dockArea: dockArea);

        foreach (Object dockArea in GuisByDockArea.Keys.ToList().Where(r => !r))
            GuisByDockArea.Remove(key: dockArea);

        foreach (TabifyGUI gui in GuisByDockArea.Values)
        {
    ...
    public static Dictionary<Object, TabifyGUI> GuisByDockArea = new();

    private static void Shortcuts() // globalEventHandler
    {
        if (!curEvent.isKeyDown()) return;
        if (EditorWindow.focusedWindow?.GetType() == GameViewType &&
            ShortcutIntegrationsType.GetMemberValue<bool>("ignoreWhenPlayModeFocused") &&
        ...
        if (curEvent.keyCode == KeyCode.T &&
            ...
            GuisByDockArea.TryGetValue(key: dockArea2, out TabifyGUI gui2))
        {
        ...
        if (curEvent.keyCode == KeyCode.W &&
            ...
            GuisByDockArea.TryGetValue(key: dockArea, out TabifyGUI gui) &&
            gui.tabs.Count != 1)
        {
        ...
        if (!GuisByDockArea.TryGetValue(key: dockArea1, out TabifyGUI gui1)) return;
    ...
    private static void UpdateBrowserTitle(EditorWindow browser)
    {
        ...
        if (isLocked)
        {
            ...
            DockAreaType.GetFieldValue<IDictionary>("s_GUIContents").Clear();
        }

        if (isLocked) return;
        if (isTitleDefault) return;

        browser.titleContent = new GUIContent("Project", EditorGUIUtility.FindTexture("Project@2x"));
        DockAreaType.GetFieldValue<IDictionary>("s_GUIContents").Clear();
    }
    ...
    private static void UpdatePropertyEditorTitle(EditorWindow propertyEditor)
    {
        ...
        if (!adjustedObjectIconsBySourceIid.TryGetValue(sourceIcon.GetInstanceID(), out Texture2D adjustedIcon))
        {
            adjustedIcon = new(width: sourceIcon.width, height: sourceIcon.height,
                textureFormat: sourceIcon.format, mipCount: sourceIcon.mipmapCount, false);
        ...
        propertyEditor.SetMemberValue("m_InspectedObject", null); // prevents further title updates from both internal code and Tabify
    ...
    public static void UpdateTitle(EditorWindow window)
    {
        ...
        bool isPropertyEditor = window.GetType() == PropertyEditorType;
        bool isBrowser = window.GetType() == ProjectBrowserType;
    ...
    private static void WrappedBrowserOnGUI(EditorWindow browser)
    {
        ...
        Rect breadcrumbsRect = headerRect.AddHeightFromBottom(-breadcrumbsYOffset * 2);
        Rect topGapRect = headerRect.SetHeight(breadcrumbsYOffset * 2);
        ...
        if (isOneColumn)
        {
            ...
            if (curEvent.isMouseDown() && browser.position.SetPos(0, 0).IsHovered())
                ProjectBrowserType.SetFieldValue("s_LastInteractedProjectBrowser", val: browser);
    ...
    private static void UpdateWrappingForBrowser(EditorWindow browser)
    {
        ...
        if (isLocked && !isWrapped)
        {
            ...
            Delegate newDelegate = typeof(Tabify).
                ...
                CreateDelegate(delegateType: EditorWindowDelegateType, target: browser);
    ...
    private static void EnsureActiveTabsVisibleOnScroller()
    {
        foreach (Object dockArea in AllDockAreas)
        {
            if (!GuisByDockArea.TryGetValue(key: dockArea, out TabifyGUI gui)) continue;
    ...
    {
        if (EditorWindow.focusedWindow?.GetType() == ProjectBrowserType)
    ...
    {
        object action = WindowActionType.GetMethod("CreateWindowMenuItem", bindingAttr: maxBindingFlags).
            Invoke(null, new[] { "Tabify dummy for creating extra space for + button", null, null });

        const float extraSpaceAmount = 21f;
        ...
        var funcDelegate = Delegate.CreateDelegate(type: ValidateHandlerType, method: mi_ShouldCreateExtraSpace);

        action.SetMemberValue("validateHandler", val: funcDelegate);

        Array defaultActions = HostViewType.GetMemberValue<Array>("s_windowActions") ??
                               HostViewType.InvokeMethod<Array>("FetchWindowActionFromAttribute");

        var newActions = Array.CreateInstance(elementType: WindowActionType, defaultActions.Length + 1);
        ...
        HostViewType.SetMemberValue("s_windowActions", val: newActions);
    }
    ...
    public class TabInfo
    {
        ...
        public bool isBrowser => typeName == ProjectBrowserType.Name;

        public bool isPropertyEditor => typeName == PropertyEditorType.Name;
        ...
        {
            isLocked = true;
            typeName = lockTo is DefaultAsset ? ProjectBrowserType.Name : PropertyEditorType.Name;
    ...
    private static void Init()
    {
        ...
        // Pre-warm the tab entry list so the first popup open is instant
    ...
    public static IEnumerable<EditorWindow> AllBrowsers => _allBrowsers ??=
        ProjectBrowserType.GetFieldValue<IList>("s_ProjectBrowsers").Cast<EditorWindow>();

    private static IEnumerable<EditorWindow> _allBrowsers;

    public static IEnumerable<EditorWindow> AllPropertyEditors => Resources.
        FindObjectsOfTypeAll(type: PropertyEditorType).
        Where(r => r.GetType().BaseType == typeof(EditorWindow)).
        Cast<EditorWindow>();

    public static List<EditorWindow> AllEditorWindows =>
        _allEditorWindows ?? typeof(EditorWindow).GetMemberValue<List<EditorWindow>>("activeEditorWindows");

    private static List<EditorWindow> _allEditorWindows;
    ...
    public static IEnumerable<Object> AllDockAreas => AllEditorWindows.Where(r => r.hasFocus && !r.maximized).
        ...
        Where(r => r.GetType() == DockAreaType);

    public static readonly Type DockAreaType = typeof(Editor).Assembly.GetType("UnityEditor.DockArea");
    public static readonly Type PropertyEditorType = typeof(Editor).Assembly.GetType("UnityEditor.PropertyEditor");
    public static readonly Type ProjectBrowserType = typeof(Editor).Assembly.GetType("UnityEditor.ProjectBrowser");
    public static readonly Type SceneHierarchyWindowType = typeof(Editor).Assembly.GetType("UnityEditor.SceneHierarchyWindow");
    public static readonly Type InspectorWindowType = typeof(Editor).Assembly.GetType("UnityEditor.InspectorWindow");
    public static readonly Type WindowActionType = typeof(Editor).Assembly.GetType("UnityEditor.WindowAction");
    public static readonly Type HostViewType = typeof(Editor).Assembly.GetType("UnityEditor.HostView");

    public static readonly Type EditorWindowDelegateType =
        HostViewType.GetNestedType("EditorWindowDelegate", bindingAttr: maxBindingFlags);

    public static readonly Type ValidateHandlerType =
        WindowActionType.GetNestedType("ValidateHandler", bindingAttr: maxBindingFlags);

    public static readonly Type EditorWindowShowButtonDelegateType =
        HostViewType.GetNestedType("EditorWindowShowButtonDelegate", bindingAttr: maxBindingFlags);

    public static readonly Type GameViewType = typeof(Editor).Assembly.GetType("UnityEditor.GameView");

    public static readonly Type ShortcutIntegrationsType =
        typeof(Editor).Assembly.GetType("UnityEditor.ShortcutManagement.ShortcutIntegration");

    public static readonly MethodInfo WrappedBrowserOnGUIMethod =
