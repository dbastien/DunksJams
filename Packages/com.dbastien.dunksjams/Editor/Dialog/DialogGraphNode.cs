...
public class DialogGraphNode : Node
    {
        ...
        private void UpdateTitle()
        {
            ...
            if (Entry.lines.Count > 0 && !string.IsNullOrEmpty(Entry.lines[0].text))
            {
                preview = Entry.lines[0].text.Length > 30
                    ? Entry.lines[0].text.Substring(0, 30) + "..."
                    : Entry.lines[0].text;
            }
            else
            {
        ...
        private void BuildPorts()
        {
            ...
            if (Entry.nodeType != DialogNodeType.End && Entry.nodeType != DialogNodeType.Comment && Entry.outgoingLinks.Count == 0)
            {
        ...
        private void BuildNodeUI()
        {
            ...
            if (Entry.nodeType == DialogNodeType.Logic || Entry.nodeType == DialogNodeType.Event ||
                Entry.nodeType == DialogNodeType.Random ||
                !string.IsNullOrEmpty(Entry.condition) || !string.IsNullOrEmpty(Entry.onExecuteScript))
            {
            ...
            if (Entry.nodeType == DialogNodeType.Dialogue || Entry.nodeType == DialogNodeType.Choice ||
                Entry.nodeType == DialogNodeType.Start || Entry.nodeType == DialogNodeType.End ||
                Entry.lines.Count > 0)
            {
                ...
                if (Entry.lines.Count == 0 && (Entry.nodeType == DialogNodeType.Dialogue || Entry.nodeType == DialogNodeType.Start))
                {
            ...
            if (Entry.nodeType != DialogNodeType.Logic && Entry.nodeType != DialogNodeType.Event &&
                Entry.nodeType != DialogNodeType.Comment && Entry.nodeType != DialogNodeType.Jump &&
                Entry.nodeType != DialogNodeType.Random)
            {
