...
public sealed partial class DLogConsole
{
    ...
    private void DrawRow(LogEntry e, int index)
    {
        ...
        if (isSelected)
        {
            Color selected = EditorGUIUtility.isProSkin
        ...
        if (e.count > 1)
        {
            Color badgeBg = EditorGUIUtility.isProSkin
    ...
    private void DrawStackTrace(LogEntry e)
    {
        ...
        {
            StackLineInfo info = e.stackLineInfos[i];
    ...
    private void OpenEntryLocation(LogEntry e)
    {
        ...
        if (DLogHub.TryExtractFileLine(e.message, e.stackTrace, out string file, out int line))
    ...
    private void OpenFileAtLine(string filePath, int line)
    {
        ...
        if (!string.IsNullOrEmpty(filePath))
        {
            ...
            if (filePath.StartsWith("Assets/")) { assetPath = filePath; }
            else
            {
