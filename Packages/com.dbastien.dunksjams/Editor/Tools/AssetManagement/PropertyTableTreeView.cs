...
internal sealed class PropertyTableModel : IDisposable
{
    ...
    private readonly Dictionary<int, TableRow> _rowsByKey = new();
    ...
    public bool Refresh()
    {
        Object[] gathered = Gather?.Invoke() ?? Array.Empty<Object>();
        ...
        for (var i = 0; i < objs.Count; i++)
        {
            ...
            if (!_rowsByKey.TryGetValue(key, out TableRow row) || row == null || row.Target != obj)
            {
...
internal sealed class PropertyTable : IDisposable
{
    ...
    public float Height { get => _height; set => _height = Mathf.Max(value, GetMinHeight()); }
    ...
    private float _height = 240f;

    // Resizer interaction
    private const float kResizerH = 18f;
    ...
    public void OnDisable() { SaveState(); }
    ...
    public void OnInspectorUpdate()
    {
        ...
        bool modelChanged = Model.Refresh();
        if (modelChanged)
            _tree.FullReload();
    ...
    public void OnGUI(Rect hostRect)
    {
        ...
        // [Tree]
        // [Resizer (optional)]
        float toolbarH = ShowToolbar ? EditorGUIUtility.singleLineHeight + 4f : 0f;
        float resizerH = ResizableHeight ? kResizerH : 0f;
    ...
    private void DrawToolbar(Rect r)
    {
        ...
        EditorGUI.BeginChangeCheck();
        bool newFilter = EditorGUI.ToggleLeft(mid, "Filter Selection", FilterToUnitySelection);
        if (EditorGUI.EndChangeCheck())
        {
        ...
        if (ShowSearch)
        {
            ...
            EditorGUI.BeginChangeCheck();
            string s = _searchField.OnGUI(searchRect, _searchText);
            if (EditorGUI.EndChangeCheck())
            {
    ...
    private void HandleResizer(Rect r)
    {
        // A centered grip.
        Rect grip = r;
        grip.height = 10f;
        ...
        switch (e.type)
        {
            ...
            case EventType.MouseDrag:
                ...
                {
                    float dy = e.mousePosition.y - _resizeStartMouseY;
    ...
    private float GetMinHeight()
    {
        ...
        float headerH = _header != null ? _header.height : 18f;
    ...
    {
        string treeJson = SessionState.GetString(_uid + ":TreeState", "");
        string headerJson = SessionState.GetString(_uid + ":HeaderState", "");
...
internal sealed class TableRow : IDisposable
{
...
internal abstract class TableColumn
{
    public string Id;
...
internal sealed class NameColumn : TableColumn
{
...
internal sealed class PropertyPathColumn : TableColumn
{
    ...
    public override bool DrawCell(Rect r, TableRow row)
    {
        ...
        SerializedProperty prop = row.Find(PropertyPath);
        if (prop == null)
        {
    ...
    public override void Apply(TableRow target, TableRow source)
    {
        ...
        SerializedProperty src = source.Find(PropertyPath);
        SerializedProperty dst = target.Find(PropertyPath);
    ...
    public override int Compare(TableRow a, TableRow b)
    {
        ...
        SerializedProperty pa = a.Find(PropertyPath);
        SerializedProperty pb = b.Find(PropertyPath);
...
internal sealed class PropertyTableTreeView : TreeView<int>
{
    ...
    public bool UpdateVisibleSerializedObjects()
    {
        ...
        {
            TableRow row = ((RowItem)_visibleItems[i]).Row;
            if (row != null)
                changed |= row.UpdateIfRequired();
        }

        // Keep selected rows fresh too (even if scrolled off)
        IList<TreeViewItem<int>> selected = FindRows(GetSelection());
        if (selected != null)
            for (var i = 0; i < selected.Count; i++)
                if (selected[i] is RowItem { Row: not null } ri)
    ...
    public bool IsFilterDirty()
    {
        ...
        int h = HashIds(_unitySelectionFilter);
    ...
    protected override IList<TreeViewItem<int>> BuildRows(TreeViewItem<int> root)
    {
        ...
        string[] tokens = Tokenize(searchString);
        ...
        if (_allItems != null)
            for (var i = 0; i < _allItems.Count; i++)
            {
                ...
                if (tokens is { Length: > 0 } && !MatchesAllTokens(row.DisplayName, tokens))
                    continue;
    ...
    protected override void RowGUI(RowGUIArgs args)
    {
        ...
        IReadOnlyList<TableColumn> cols = _table.GetColumns();
        for (var vis = 0; vis < args.GetNumVisibleColumns(); vis++)
        {
            int colIndex = args.GetColumn(vis);
            TableColumn col = cols[colIndex];

            Rect cell = args.GetCellRect(vis);
            CenterRectSingleLine(ref cell);

            var changed = false;

            EditorGUI.BeginDisabledGroup(!IsEditable(item.Row.Target));
            try { changed = col.DrawCell(cell, item.Row); }
    ...
    private void HandleCellEdited(TableRow editedRow, int columnIndex)
    {
        ...
        ApplyMode mode = _table.GetApplyMode();
        ...
        {
            IEnumerable<TableRow> others = Enumerable.Empty<TableRow>();

            switch (mode)
            {
                case ApplyMode.SelectedRows:
                {
                    IList<int> selIds = GetSelection();
                    if (selIds is { Count: > 0 })
                    {
                        IList<TreeViewItem<int>> selRows = FindRows(selIds);
                        if (selRows != null)
        ...
        Object[] undoTargets = targets.Select(t => t.Target).Where(o => o != null).Distinct().ToArray();
    ...
    private void EnsureItemsBuilt()
    {
        ...
        IReadOnlyList<TableRow> rows = _table.GetAllRows();
    ...
    private void ApplySorting(List<TreeViewItem<int>> rows)
    {
        ...
        int sortedCol = multiColumnHeader.sortedColumnIndex;
        if (sortedCol < 0)
            return;
        ...
        TableColumn col = cols[sortedCol];
        if (!col.Sortable)
            return;

        bool asc = multiColumnHeader.IsSortedAscending(sortedCol);

        rows.Sort((a, b) =>
        {
            TableRow ra = (a as RowItem)?.Row;
            TableRow rb = (b as RowItem)?.Row;
            int cmp = col.Compare(ra, rb);
            return asc ? cmp : -cmp;
        });
    }

    private static void CenterRectSingleLine(ref Rect r)
    {
    ...
    private static bool IsEditable(Object target) => target != null && (target.hideFlags & HideFlags.NotEditable) == 0;
    ...
    private static bool MatchesAllTokens(string hay, string[] tokens)
    {
...
internal static class SerializedObjectUtil
{
    ...
    private static MethodInfo _disposeMI;
...
internal static class PropertyCopyUtil
{
    private static bool _copyChecked;
    private static Action<SerializedProperty, SerializedProperty> _copyFromProp; // open instance delegate

    private static void EnsureCopyDelegate()
    {
        ...
        MethodInfo mi = typeof(SerializedProperty).GetMethod(
