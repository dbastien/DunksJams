...
internal sealed class PropertyTableModel : IDisposable
{
    ...
    private readonly Dictionary<int, TableRow> _rowsByKey = new Dictionary<int, TableRow>();
...
internal sealed class PropertyTable : IDisposable
{
    ...
    {
        _uid = string.IsNullOrEmpty(uid) ? throw new ArgumentException("uid must be stable and non-empty", nameof(uid)) : uid;
    ...
    public void OnGUI(Rect hostRect)
    {
        ...
        var treeRect = new Rect(hostRect.x, hostRect.y + toolbarH, hostRect.width, Mathf.Max(0f, hostRect.height - toolbarH - resizerH));
...
internal sealed class PropertyTableTreeView : TreeView
{
    ...
    protected override void RowGUI(RowGUIArgs args)
    {
        ...
        for (int vis = 0; vis < args.GetNumVisibleColumns(); vis++)
        {
            ...
            if (changed)
            {
                HandleCellEdited(item.Row, colIndex);
            }
        }
    }
    ...
    {
        return target != null && ((target.hideFlags & HideFlags.NotEditable) == 0);
    }
...
internal static class SerializedObjectUtil
{
    ...
    public static void TryDispose(SerializedObject so)
    {
        ...
        {
            _checked = true;
            _disposeMI = typeof(SerializedObject).GetMethod("Dispose", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
        }
