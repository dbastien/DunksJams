...
{
    [NormalizedAnimationCurve] public AnimationCurve vertexCurve = new(new Keyframe(0, 0), new Keyframe(0.25f, 1.0f), new Keyframe(0.5f, 0), new Keyframe(0.75f, 1.0f), new Keyframe(1, 0f));
    ...
    [NormalizedAnimationCurve] public AnimationCurve rotationCurve = new(new Keyframe(0, 0), new Keyframe(0.25f, 1.0f), new Keyframe(0.5f, 0), new Keyframe(0.75f, 1.0f), new Keyframe(1, 0f));   
    [Range(0.001f, .25f)] public float rotationScale = 0.05f;

    float _timeElapsed;

    TMP_Text _textComponent;
    TMP_MeshInfo[] _initialMeshInfo;
    ...
    public void Update()
    {
        ...
        for (var i = 0; i < characterCount; ++i)
        {
            ...
            float x0 = (offsetToMidBaseline.x - boundsMinX) / boundsDelta; // Character's position relative to the bounds of the mesh.
            ...
            if (rotateLetters)
            {
                ...
                matrix = Matrix4x4.Translate(offsetToMidBaseline) *
                         Matrix4x4.TRS(new(0, y0, 0), q, Vector3.one) *
                         Matrix4x4.Translate(-offsetToMidBaseline);
            }
            else
            {
                matrix = Matrix4x4.Translate(new(0, y0, 0));
            }
