...
{
    public enum FibonacciStructureType { Spiral, RadialStack, Grid, RandomOffset }
    ...
    public static Vector3 InCuboid(float width, float height, float depth) =>
        new(Rand.Float() * width - width * 0.5f, Rand.Float() * height - height * 0.5f, Rand.Float() * depth - depth * 0.5f);
    ...
    public static Vector3 InCone(float rad, float spread)
    {
        ...
        return new(sinSpread * MathF.Cos(angle), sinSpread * MathF.Sin(angle), cosSpread * rad);
    }
    ...
    public static Vector3 InHemisphere(float radius)
    {
        ...
        return new(r * sinPhi * MathF.Cos(theta), r * sinPhi * MathF.Sin(theta), r * MathF.Cos(phi));
    }
    ...
    public static Vector3 InTorus(float majorR, float minorR)
    {
        ...
        return new((majorR + r) * MathF.Cos(theta), (majorR + r) * MathF.Sin(theta), r * MathF.Sin(phi));
    }
    ...
    public static Vector3 OnTorus(float majorR, float minorR)
    {
        float theta = Rand.Rad();
        float phi = Rand.Rad();
        return new((majorR + minorR * MathF.Cos(phi)) * MathF.Cos(theta), (majorR + minorR * MathF.Cos(phi)) * MathF.Sin(theta), minorR * MathF.Sin(phi));
    }
    ...
    public static Vector3 InTorusKnot(float p, float q, float radius, float tubeR)
    {
        ...
        return new(r * MathF.Cos(p * theta), r * MathF.Sin(p * theta), tubeR * MathF.Cos(q * theta));
    }

    public static Vector3 OnTorusKnot(float p, float q, float r, float tubeR)
    {
        float theta = Rand.Rad();
        return new((tubeR * MathF.Cos(q * theta) + r) * MathF.Cos(p * theta), (tubeR * MathF.Cos(q * theta) + r) * MathF.Sin(p * theta), tubeR * MathF.Sin(q * theta));
    }
    ...
    public static Vector3 InSpiral(float radius, float height, float revolutions)
    {
        ...
        return new(r * MathF.Cos(theta), r * MathF.Sin(theta), Rand.Float() * height - height * 0.5f);
    }
    ...
    public static Vector3 InHelix(float radius, float height, float revolutions, float pitch)
    {
        float theta = Rand.Rad() * revolutions;
        float r = radius * theta / MathConsts.Tau;
        return new(r * MathF.Cos(theta), r * MathF.Sin(theta), pitch * theta / MathConsts.Tau);
    }

    public static Vector3 OnHelix(float r, float height, float revolutions, float pitch)
    {
        float theta = Rand.Rad() * revolutions;
        return new(r * theta / MathConsts.Tau * MathF.Cos(theta), r * theta / MathConsts.Tau * MathF.Sin(theta), pitch * theta / MathConsts.Tau);
    }
    ...
    public static Vector3 InFibonacciSphere(float radius, int idx)
    {
        ...
        return new(r * MathF.Cos(phi), y * radius, r * MathF.Sin(phi));
    }
    ...
    public static Vector3 InFibonacciStructure3D(float radius, int index, FibonacciStructureType type)
    {
        ...
        return type switch
        {
            FibonacciStructureType.Spiral => new(r * MathF.Cos(phi), y * radius, r * MathF.Sin(phi)),
            FibonacciStructureType.RadialStack => new(r * MathF.Cos(phi), r * MathF.Sin(phi), index * 0.1f), // Stacks along z
            FibonacciStructureType.Grid => new(r * MathF.Cos(phi), y * radius, r * MathF.Sin(phi)), // Similar to lattice, but less aligned
            FibonacciStructureType.RandomOffset => new(r * MathF.Cos(phi + Rand.Float() * 0.1f),
